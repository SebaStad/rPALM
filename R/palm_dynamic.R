...
#' @examples
  ...
  set_initial_profile_lod1 = function(variable, data.vector){
    ...
    for(i in seq(loopvar)){
      variable <- loopvar[i]
      adata <- list("long_name" = paste0("Initial profile of ", variable),
  ...
  set_forcing_lateral = function(variable, direction, data.vector){
    if(direction=="north"){
      namevar <- "north"
      cor.axis <- "x"
     ...
     print("Only lateral sides can be done this way!")
     print("Top Boundary by set_forcing_top")
    ...
    second_variable <- self$palmgrid[[variable]][which(cor.axis%in%self$palmgrid[[variable]])]


    data.vector.rep  <- rep(data.vector,length(self$dims$time$vals)*length(self$dims[[second_variable]]$vals))
    data.vector.time <- array(data.vector.rep,c(length(data.vector),
                                                length(self$dims[[second_variable]]$vals),
                                                length(self$dims$time$vals)
                                                )
    )
    ...
    for(i in seq(loopvar)){
      ...
      adata <- list("long_name" = paste0("large scale forcing for ", variable),
                    ...
                    "coordinates" = "E_UTM N_UTM lon lat",
                    "grid_mapping" = "crsUTM: E_UTM N_UTM crsETRS: lon lat",
                    "vals" = data.vector.time)

      whichdimensions  <- c("z",second_variable, "time")
  ...
  create_dummy_vars = function(){
    ...
    adata <- list("long_name" = "dummy for zw",
                  "source" = "Synthethic data from calculated profile",
                  "units" = "K",
                  "_FillValue" = -9999.9,
                  "type" = "float",
    ...
    adata <- list("long_name" = "dummy for xu",
                  "source" = "Synthethic data from calculated profile",
                  "units" = "K",
                  "_FillValue" = -9999.9,
                  "type" = "float",
    ...
    adata <- list("long_name" = "dummy for yv",
                  "source" = "Synthethic data from calculated profile",
                  "units" = "K",
                  "_FillValue" = -9999.9,
                  "type" = "float",
